...
namespace ZXing.QrCode.Internal
{
   ...
   {
      //private const string kPrefixBinary = "";
      //private const int kPrefixLength = 0;
      private const string kPrefixBinary = "#LSAD";
      private const int kPrefixLength = 7;
                                            ...
                                            IDictionary<DecodeHintType, object> hints)
      {
         ...
         try
         {
            if (kPrefixBinary.Length > 0)
            {
               if (result.ToString().IndexOf(kPrefixBinary) == 0)
               {
                  if (result.Length > kPrefixLength)
                  {
                     int nTmp = result.Length - kPrefixLength;
                     string tmpString = result.ToString().Substring(kPrefixLength, nTmp);
                     byte[] tmpbuf = new byte[nTmp + count];
                     Buffer.BlockCopy(Encoding.GetEncoding("ASCII").GetBytes(tmpString), 0, tmpbuf, 0, nTmp);
                     Buffer.BlockCopy(readBytes, 0, tmpbuf, nTmp, count);
                     result.Remove(kPrefixLength, nTmp);
